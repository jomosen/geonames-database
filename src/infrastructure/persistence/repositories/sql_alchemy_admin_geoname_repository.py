...
class SqlAlchemyAdminGeoNameRepository(SqlAlchemyGeoNameRepository, AbstractGeoNameRepository):

    def count_all(self) -> int:
        count = self.session.query(GeoNameModel).where(GeoNameModel.feature_class == "A").count()
        return count
